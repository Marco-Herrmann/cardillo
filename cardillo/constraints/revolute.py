...
class Revolute(PositionOrientationBase):
    ...
    def KN_l(self, t, q, la_l):
        # compre definition of g: ea_1 @ eb_2 = 0
        # here for small l (angle): l = y = ea_2 @ eb_1
        nu1 = self._nu1
        ...
        a, b = self.plane_axes
        ea, eb = A_IJ2[:, a], A_IJ2[:, b]
        n = cross3(ea, eb)

        ea_tilde = ax2skew(ea)
        eb_tilde = ax2skew(eb)

        Dn1 = ea_tilde @ eb_tilde @ J_R1
        Dn2 = -eb_tilde @ ea_tilde @ J_R2
        DW_l[:nu1, :nu1] = J_R1.T @ Dn1 - np.einsum("i,ijk->jk", n, DJ_R1)
        DW_l[:nu1, nu1:] = J_R1.T @ Dn2
        DW_l[nu1:, :nu1] = -J_R2.T @ Dn1
        DW_l[nu1:, nu1:] = -(J_R2.T @ Dn2 - np.einsum("i,ijk->jk", n, DJ_R2))

        return -la_l * DW_l, np.zeros_like(DW_l)
