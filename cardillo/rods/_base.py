...
class CosseratRod_PetrovGalerkin(RodExportBase, ABC):
    ...
    def element_interval(self, el):
        return self.knot_vector_r.element_interval(el)
    ...
    def q_dot(self, t, q, u):
        ...
        for node in range(self.nnodes_p):
            ...
            q_dot[nodalDOF_p] = T_SO3_inv_quat(p, normalize=False) @ B_omega_IB
...
class CosseratRodMixed(CosseratRod_PetrovGalerkin):
    def __init__(
        ...
        *,
        ...
        self.nmixed_n = len(self.mixed_n)

        self.mixed_m = self.idx_mixed[(self.idx_mixed >= 3)] - 3
        self.nmixed_m = len(self.mixed_m)
        ...
        # global nodal connectivity
        self.nodalDOF_la_c = self.mesh_la_c.nodalDOF
    ...
    def c_el(self, qe, la_ce, el):
        ...
        c_el = np.zeros(self.nla_c_element, dtype=dtype)
        ...
        for i in range(self.nquadrature):
            ...
            la_c = np.zeros(self.nmixed, dtype=dtype)
            # interpolation of internal forces and moments
            for node in range(self.nnodes_element_la_c):
                la_c_node = la_ce[self.nodalDOF_element_la_c[node]]
                la_c += self.N_la_c[el, i, node] * la_c_node
            ...
            for node_p in range(self.nnodes_element_p):
                ...
                for node_la_c in range(self.nnodes_element_la_c):
                    ...
                    W_c_el[nodalDOF_p[:, None], nodalDOF_la_c[self.nmixed_n :]] -= (
                        (N_p_xi * eye3 - N_p * ax2skew(B_Kappa_bar))[:, self.mixed_m]
                        * self.N_la_c[el, i, node_la_c]
                        * qwi
                    )
                    ...
                    W_c_el[nodalDOF_p[:, None], nodalDOF_la_c[: self.nmixed_n]] += (
                        N_p
                        * ax2skew(B_Gamma_bar)[:, self.mixed_n]
                        * self.N_la_c[el, i, node_la_c]
                        * qwi
                    )
    ...
    def Wla_c_el_qe(self, qe, la_ce, el):
        dtype = np.common_type(qe, la_ce)
        Wla_c_el_qe = np.zeros((self.nu_element, self.nq_element), dtype=dtype)
    ...
    def _eval_stresses_MX(self, la_c, xi, el=None):
        ...
        B_n = np.zeros(3, dtype=float)
        B_m = np.zeros(3, dtype=float)

        B_n[self.mixed_n] = la_cc[: self.nmixed_n]
        B_m[self.mixed_m] = la_cc[self.nmixed_n :]
    ...
    class CosseratRodConstrained(CosseratRodBase):
        def __init__(
            ...
            *,
            ...
            ##################################################################
            self.idx_constraints = np.array(idx_constraints)
            self.nconstraints = len(self.idx_constraints)

            self.constraints_gamma = self.idx_constraints[(self.idx_constraints < 3)]
            self.nconstraints_gamma = len(self.constraints_gamma)

            self.constraints_kappa = (
                self.idx_constraints[(self.idx_constraints >= 3)] - 3
            )
            ...
            self.mesh_la_g = Mesh1D(
                ...
                nquadrature,
                dim_q=len(self.idx_constraints),
                derivative_order=0,
                basis="Lagrange_Disc",
                quadrature="Gauss",
                dim_u=len(self.idx_constraints),
            )
            ...
            # global element connectivity for constraint coordinates
            self.elDOF_la_g = self.mesh_la_g.elDOF

            # global nodal connectivity
            self.nodalDOF_la_g = self.mesh_la_g.nodalDOF
        ...
        def g_el(self, qe, el):
            ...
            for i in range(self.nquadrature):
                ...
                g_qp = np.concatenate(
                    [
                        (B_Gamma_bar - Ji * B_Gamma0),
                        (B_Kappa_bar - Ji * B_Kappa0),
                    ]
                )

                for node in range(self.nnodes_element_la_g):
                    g_el[self.nodalDOF_element_la_g[node]] -= (
                        self.N_la_g[el, i, node] * g_qp[self.idx_constraints] * qwi
                    )
            ...
            for i in range(self.nquadrature):
                ...
                for node in range(self.nnodes_element_la_g):
                    ...
                    g_q_el[nodalDOF_la_g, :] -= (
                        ...
                        * delta_strains_qe[self.idx_constraints, :]
                        * qwi
                    )
                ...
                for node_p in range(self.nnodes_element_p):
                    ...
                    for node_la_g in range(self.nnodes_element_la_g):
                        ...
                        W_g_el[
                            ...
                            (N_p_xi * eye3 - N_p * ax2skew(B_Kappa_bar))[
                                :, self.constraints_kappa
                            ]
                            * self.N_la_g[el, i, node_la_g]
                            * qwi
        ...
        def Wla_g_q_el(self, qe, la_ge, el):
            ...
            Wla_g_q_el = np.zeros((self.nu_element, self.nq_element), dtype=dtype)
            ...
            for i in range(self.nquadrature):
                ...
                # interpolation of the n and m
                la_g = np.zeros(self.nconstraints, dtype=dtype)
        ...
        def eval_stresses(self, t, q, la_c, la_g, xi, el=None):
            ...
            la_gg = np.zeros(self.nconstraints, dtype=float)
