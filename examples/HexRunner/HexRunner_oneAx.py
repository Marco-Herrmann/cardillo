...
if __name__ == "__main__":
    ...
    cylinder1.mass = 2.0
    cylinder1.B_Theta_C = np.diag([0.043, 0.043, 0.0025])
    ...
    cylinder2.mass = 1.0
    cylinder2.B_Theta_C = np.diag([0.01065, 0.01065, 0.00125])
    ...
    # motor = Motor(revolute_12, lambda t: 0.0)
    system.add(motor)
    ...
    for i, (alpha0, parent) in enumerate(zip([0, 60], [cylinder2, cylinder2])):
        for j in range(3):
            ...
            cylinderi.mass = 0.05
            cylinderi.B_Theta_C = np.diag([2.62e-4, 2.62e-4, 2.5e-6])
